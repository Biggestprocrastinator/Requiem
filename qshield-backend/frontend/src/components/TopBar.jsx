...
export default function TopBar({ onScan, onStopScan, isScanning = false }) {
  ...
  const canRunScan = canAccess(role, 'scan');
    ...
    <header className="fixed top-0 right-0 left-64 h-16 flex items-center justify-between px-6 z-40 bg-white/80 backdrop-blur-xl border-b border-outline-variant/40 shadow-[0_4px_24px_rgba(15,23,42,0.02)]">
      ...
      <div className="flex items-center gap-4">
        {canRunScan ? (
          ...
          <div className="px-4 py-2 rounded-full border border-outline-variant/40 bg-surface-container/70 text-xs text-on-surface-variant flex items-center gap-2">
            <span className="material-symbols-outlined text-base">lock</span>
            <span>Vulnerability scanning reserved for Admin / IT Admin.</span>
          </div>
        )}
